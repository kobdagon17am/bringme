...
class API1Controller extends Controller
{
    ...
    public function api_add_cart(Request $r)
    {
        ...
        try {
            ...
            $customer_address = Customer_address::select('customer_address.id')
    ...
    public function api_get_home_page(Request $r)
    {
        ...
        $customer = [];
        if ($r->user_id != 0) {
            $customer_address = Customer_address::select('customer_address.*', 'customer_address.district_id as districts_name', 'amphures.name_th as amphures_name', 'provinces.name_th as provinces_name')
    ...
    public function api_get_product_detail(Request $r)
    {
        ...
        if ($product_detail) {
            ...
            $products_comment = ProductsComment::select('products_comment.*', 'customer.name as cus_name')
                            ...
                            if ($p->pre_order_status == 0) {
                                ...
                                foreach ($stock_lot_arr as $st_arr) {
                                    if ($qty_mis_total > 0) {
                                        ...
                                        $stock_lot->qty_booking -= $p->qty;
                                        $stock_lot->save();

                                        $stock_items = StockItems::where('product_id', $p->product_id)->where('stock_lot_id', $stock_lot->id)->first();
                                        $stock_items->qty_booking -= $customer_cart_product_cut_stock->qty_has;
                                        $stock_items->save();
                                    }
                                }
                            } else {
